...
def get_schedule(request):
    ...
    #headers = ['0_time', 'Discord username of Volunteer', 'Action in Location', 'Command or TXT', 'Done']
    #schedule.writerow(headers)
    ...
    for sft in Shift.objects.all():
        ...
        schedule.writerow([
            pretime,
            sft.discord_username,
            f"Start Shift in {sft.location}",
            f"{sft.name} - {sft.email}",""
        ])
        schedule.writerow([
            sft.end,
            sft.discord_username,
